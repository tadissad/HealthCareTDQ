...
# Build KB hợp nhất cho ai-service (sources nằm trong ai-service/kb/sources)
python ai-service/kb/build_unified_kb.py --target-products 50
...
| "Thiếu API Key" | Kiểm tra file `.env`, make sure `GEMINI_API_KEY` được set. Restart container: `docker-compose restart ai-service` |
