...
class OrderCreate(APIView):
    ...
    def post(self, request):
        ...
        for item in cart_items:
            ...
            name  = item.get('name', f"Sản phẩm #{item.get('product_id')}")
        ...
        # 3. Áp dụng giảm giá và lưu tổng tiền
        ...
        try:
            patient_r = requests.get(f"{PATIENT_SERVICE_URL}/patients/by-account/{customer_id}/", timeout=5)
            if patient_r.status_code == 200:
                current_spent = float(patient_r.json().get('total_spent', 0))
                new_spent     = current_spent + float(order.total_amount)
                requests.put(
                    f"{PATIENT_SERVICE_URL}/patients/by-account/{customer_id}/membership/",
                    json={'total_spent': new_spent},
                    timeout=5,
                )
        except Exception:
            pass
